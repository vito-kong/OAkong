...
import com.dynastech.cdmetro.beans.FormFile;
import com.dynastech.cdmetro.beans.Upload;
...
public class SocketHttpRequester {
	...
	public static boolean post(String path, Map<String, String> params,
			FormFile[] files,Upload item) throws Exception {
		...
		URL url = new URL(path);
		Log.i("SocketHttpRequester", "path:" + path);
		...
		for (FormFile uploadFile : files) {
			...
			if (uploadFile.getInStream() != null) {
				...
				item.setState(ConfigInfo.UPLOADSTATE.BEGIN);
				while ((len = uploadFile.getInStream().read(buffer, 0,
					...
					if (ConfigInfo.UPLOADSTATE.CANCELED.equals(item.getState())) {
							break;
					}
				}

				if (!ConfigInfo.UPLOADSTATE.CANCELED.equals(item.getState())
						&& !ConfigInfo.UPLOADSTATE.FAILED.equals(item
								.getState())) {
					item.setState(ConfigInfo.UPLOADSTATE.FINISHED);
				}
	 ...
	 */
	public static boolean post(String path, Map<String, String> params,
			FormFile file, Upload item) throws Exception {
		return post(path, params, new FormFile[] { file }, item);
	}
